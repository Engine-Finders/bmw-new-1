...
function BuyerRow({ row, index }) {
  ...
  return (
    <article className="grid overflow-hidden rounded-md border border-[var(--color-border)] bg-[var(--color-surface-raised)] shadow-[0_10px_26px_var(--color-shadow)] md:rounded-none md:border-0 md:border-t md:shadow-none lg:grid-cols-[40%_18%_42%]">
      <div className="grid grid-cols-[31%_69%] md:grid-cols-[300px_1fr] lg:border-r lg:border-[var(--color-border)]">
        ...
        <div className="flex min-w-0 items-center gap-4 px-4 py-4 md:px-7">
          ...
          <div className="min-w-0">
            <h3 className="text-[18px] font-bold leading-[1.18] text-[var(--color-text)] md:text-[22px]" dangerouslySetInnerHTML={{ __html: cleanText(row.buyerProfile) }} />
      ...
      <p className={`border-t border-[var(--color-border)] px-4 py-4 text-[var(--color-text)] md:px-7 md:py-5 lg:border-t-0 ${sectionTableText} md:text-[18px]`} dangerouslySetInnerHTML={{ __html: cleanText(row.verdict) }} />
    </article>
  );
}
